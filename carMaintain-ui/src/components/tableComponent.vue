...
<div id="tablePage">
        ...
        <a-input-group compact class="searchItem" v-for="(itemValue,itemIndex) in tableConfig.searchConfig.select" :key="itemIndex">
            <a-input v-model:value="itemValue.selectKey" readonly>
                <template v-slot:addonAfter>
                    <a-select style="width:220px" :style="{'color':itemValue.selectValue!='全部'?'#40a9ff':'#bfbfc5'}" v-if="$store.getters['customData/publicDataOver'] || !itemValue.requestKey"
                     v-model:value="itemValue.selectValue" showSearch allowClear @change="(c,option)=>selectChange_search({changeData:c,itemValue,option})" placeholder="全部">
                        <a-select-option :value="selectItem+selectIndex" v-for="(selectItem,selectIndex) in enumObj.Enum.enumType[itemValue.selectType]" :key="selectIndex">
                            {{selectItem}}
                        </a-select-option>
                    </a-select>
    ...
    <div class="operation_contains" v-if="tableConfig.operationConfig">
        <span class="operation" v-for="(item,index) in tableConfig.operationConfig" :key="index" :style="{'float':item.float?item.float:'left','margin':item.float=='left' || !item.float?'0px 20px 0px 0px':'0px 0px 0px 20px'}">
            <a-button
            type="primary" 
            ...
            <div :key="index"  @dblclick="copyText(item,record,$event)">
                <div v-if="item.slots" style="display: flex;justify-content: center;align-items: center;" :style="{'min-height':minHeight+'px'}">
                    ...
                    <div v-else-if="item.rowType=='img'"  :style="{'height':item.config.height+20+'px','width':item.config.width+'px'}" class="asdasdasd">
                        <!-- <img :src="text" alt="" :style="{'width':item.config.width+'px'}" @click="clickItem(text,record,item)"> -->
                    ...
                    <div v-else-if="!record.editAllTable && item.rowType=='operation'" style="display:flex">
                        <div v-for="(operation,oIndex) in item.operationList" :key="oIndex">
                            ...
                            <div v-if="!operation.role || operation.role.includes(userInfo.role)">
                                ...
                                <!--删除按钮-->
                                <a-button v-if="operation.clickType=='delete' && operation.needCustomDisable===true && (record.is_sub===1 ||(record.refund_state && record.refund_state!=0))" size="small" type="danger" disabled="true">{{operation.text}}</a-button>
                                <a-popconfirm v-else-if="operation.clickType=='delete'" :title="operation.title" @confirm="clickItem(text,record,item,operation)">
                                    <a-button size="small" type="danger" :disabled="record.is_sub===1">{{operation.text}}</a-button>
                                </a-popconfirm>
...
export default {
    ...
    methods: {
        ...
        editAll(text, record, item, operation) {
        ...
        initSearchForm() {
            ...
            this.create_date = this.tableConfig.searchConfig.isToday ? [moment().format("YYYY-MM-DD"), moment().format("YYYY-MM-DD")] : []
            this.create_date = this.tableConfig.searchConfig.isWeek ? [moment().subtract(7, 'day').format("YYYY-MM-DD"), moment().format("YYYY-MM-DD")] : []
        ...
        selectChange_search({itemValue,changeData,option}) {
            ...
            this.searchForm[tempKeyName] = itemValue.selectValue ? parseInt(option.props.key) : "";
            this.pageCache[this.$route.name] = this.searchForm
            setCache("searchForm",this.pageCache,sessionStorage)
            console.log("下拉输入搜索框",this.searchForm)
        },
        ...
        editOrAdd({isAddData,modalTitle,modalWidth,openType,ajaxInterface,ajaxMethod,paramsType,extParams}) {
            console.log("晚上， ",{isAddData,modalTitle,modalWidth,openType,ajaxInterface,ajaxMethod,paramsType,extParams})
            if (openType == "modal") {
        ...
        search() {
            console.log("this.pageCache",this.pageCache)
            this.pageCache[this.$route.name] = this.searchForm;
            setCache("searchForm",this.pageCache,sessionStorage);
            this.fetch()
        },
        handlerCacheSearch(){
            ...
            console.log("获取缓存的搜索条件",this.pageCache)
